...
function processFile(filePath) {
  ...
  const regex1 = /return\s+\{\s*success\s*:\s*false\s*,\s*error\s*:\s*(?:String\(e\)|String\(err\)|\(e as Error\)\.message|\(err as Error\)\.message)\s*\}\s*;/g;
  ...
  const regex2 = /catch\s*\(\s*(e|err|error)\s*\)\s*\{\s*return\s*\{\s*success\s*:\s*false\s*,\s*error\s*:\s*String\(\1\)\s*\}\s*;\s*\}/g;
  ...
  content = content.replace(/return\s+\{\s*success\s*:\s*false\s*,\s*error\s*:\s*String\((e|err|error)\)\s*\}\s*;/g, 'return handleApiError($1);');
  content = content.replace(/return\s+\{\s*success\s*:\s*false\s*,\s*error\s*:\s*\(\s*(e|err|error)\s*as\s*Error\s*\)\.message\s*\}\s*;/g, 'return handleApiError($1);');
