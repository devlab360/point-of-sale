...
posContent = posContent.replace(/import \{ localDb, addSystemNotification \} from "@\/lib\/db";\n/, '');
posContent = posContent.replace(/import \{ useLiveQuery \} from "dexie-react-hooks";\n/, '');
// Fix pos.tsx 'c' unknown error:
posContent = posContent.replace(/Array\.from\(new Map\(categories\.map\(c => \[c\.name\.trim\(\)\.toLowerCase\(\), c\]\)\)\.values\(\)\)\.map\(c =>/g, 
...
uApi = uApi.replace(/token: z\.string\(\), role: z\.string\(\), permissions: z\.array\(z\.string\(\)\)\.optional\(\), invitation: z\.object\(\{/g, 'invitation: z.object({'); // clean up previous bad replace
uApi = uApi.replace(
...
for (const file of files) {
  ...
  c = c.replace(/\.omit\(\{ id: true \}\)\.passthrough\(\)/g, '.omit({ id: true }).partial().passthrough()');
