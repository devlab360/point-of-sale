...
async function runDatabaseExplainAnalyze() {
  ...
  const queriesToTest = [
    {
      table: "sales",
      query: sql`EXPLAIN (ANALYZE, BUFFERS, FORMAT JSON) SELECT * FROM sales WHERE organization_id = ${testOrgId}`,
    },
    {
      table: "sale_items",
      query: sql`EXPLAIN (ANALYZE, BUFFERS, FORMAT JSON) SELECT * FROM sale_items WHERE organization_id = ${testOrgId}`,
    },
    {
      table: "products",
      query: sql`EXPLAIN (ANALYZE, BUFFERS, FORMAT JSON) SELECT * FROM products WHERE organization_id = ${testOrgId}`,
    },
    {
      table: "customers",
      query: sql`EXPLAIN (ANALYZE, BUFFERS, FORMAT JSON) SELECT * FROM customers WHERE organization_id = ${testOrgId}`,
    },
    {
      table: "inventory_movements",
      query: sql`EXPLAIN (ANALYZE, BUFFERS, FORMAT JSON) SELECT * FROM inventory_movements WHERE organization_id = ${testOrgId}`,
    },
  ];
  ...
  for (const item of queriesToTest) {
    try {
      ...
      console.log(
        `✅ EXPLAIN ${item.table}: Node Type = ${queryResult.nodeType} | Seq Scan? ${isSeqScan ? "🚨 YES (Unindexed)" : "NO"} | Exec Time: ${queryResult.executionTimeMs}ms | Cost: ${queryResult.totalCost}`,
      );
    } catch (err) {
...
function runMemoryAndCpuProfiling() {
  ...
  const mockData = {
    ...
    saleItems: Array.from({ length: 20000 }, (_, i) => ({
      id: i,
      organizationId: "org-1",
      saleId: `sale-${Math.floor(i / 2)}`,
      productId: `prod-${i % 5000}`,
      price: "75.00",
      total: "75.00",
    })),
...
async function runHttpConcurrencyBenchmark() {
  ...
  if (!activePort) {
    console.log(
      "⚠️ Local dev server not reachable on 5173, 3000, or 8080. Skipping HTTP concurrency probe.",
    );
    results.http_concurrency_benchmark = {
      status: "Server not running or unreachable during script execution",
    };
    return;
  }
  ...
  const payload = JSON.stringify({
    data: { orgId: "test-load-org-123", syncKey: "default-sync-key" },
  });
  ...
  for (const concurrency of concurrencyLevels) {
    ...
    console.log(
      `⚡ Concurrency ${concurrency}: Avg Latency = ${avgLatencyMs}ms | Measured RPS = ${rps} | Success: ${successful}/${concurrency}`,
    );
  }
