...
export function BundleManager({ components, onChange, basePrice, onAutoSumPrice }: BundleManagerProps) {
  const { data: productsRes } = useQuery({ 
    queryKey: ["products"], 
    queryFn: () => getProductsFn({ data: { page: 1, pageSize: 1000, query: "" } }).then(res => res?.data || []) 
  });
      ...
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          ...
          <p className="text-xs text-muted-foreground">
            Select items that make up this bundle. Inventory will be deducted from these components upon sale.
          </p>
        </div>
      ...
      <div className="space-y-3 mt-4">
        {components.length === 0 ? (
          ...
          components.map((comp, index) => {
             ...
             return (
              <div key={index} className="flex items-start gap-3 p-3 bg-white border rounded-md shadow-sm">
                ...
                {selectedProduct?.hasVariants && (
                  <div className="flex-1 space-y-1">
                    ...
                    <SearchableSelect
                      options={selectedProduct.variants.map((v: any) => ({ label: v.name, value: v.id }))}
                ...
                <div className="pt-6">
                  <Button type="button" variant="ghost" size="icon" className="text-red-500 hover:text-red-700 hover:bg-red-50" onClick={() => removeComponent(index)}>
      ...
      {components.length > 0 && (
        <div className="flex items-center justify-between pt-4 border-t">
          <div className="text-sm text-muted-foreground">
            Current Bundle Price: <span className="font-medium text-foreground">{formatCurrency(basePrice)}</span>
          </div>
