...
export function VariantManager({ variants, onChange, mode = "product" }: VariantManagerProps) {
  ...
  useEffect(() => {
    ...
    const isDifferent = newVariants.length !== variants.length || 
    ...
    <div className="space-y-6 border rounded-xl p-5 bg-muted/10 shadow-sm">
      <div className="flex flex-col space-y-4 md:flex-row md:justify-between md:items-start md:space-y-0">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Define your options (e.g. Size, Color) and values (e.g. S, M, Red). We'll automatically generate the combinations.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {options.map((opt, optIndex) => (
          <div key={optIndex} className="bg-background rounded-lg border p-4 space-y-3 relative group transition-all hover:border-primary/30">
            <Button 
              type="button" 
              variant="ghost" 
              size="icon" 
            ...
            <div className="flex flex-wrap gap-2 items-center">
              {opt.values.map((val, valIndex) => (
                <div key={valIndex} className="flex items-center gap-1 bg-primary text-primary-foreground px-3 py-1 rounded-full text-sm font-medium shadow-sm">
                  {val}
                  <button 
                    type="button"
        ...
        <div className="pt-6 border-t">
          <div className="mb-4">
            <Label className="text-sm font-semibold flex items-center gap-2">
              Generated Matrix <span className="bg-primary/10 text-primary px-2 py-0.5 rounded-full text-xs">{variants.length}</span>
            </Label>
            <p className="text-xs text-muted-foreground mt-1">
              ...
              <table className="w-full text-sm text-left">
                <thead className="bg-muted/50 text-muted-foreground text-xs uppercase font-semibold">
                  <tr>
                    ...
                    {mode === "product" && (
                      <th className="px-4 py-3 w-48">SKU (Optional)</th>
                    )}
                    {mode === "service" && (
                      <th className="px-4 py-3 w-48">Duration (Mins)</th>
                    )}
                  </tr>
                </thead>
