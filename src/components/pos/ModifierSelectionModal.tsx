...
export function ModifierSelectionModal({ product, isOpen, onClose, onConfirm }: ModifierSelectionModalProps) {
  ...
  const handleToggleMultiple = (group: any, option: any, checked: boolean) => {
    if (checked) {
      setSelected(prev => [...prev, { id: group.id, name: group.name, optionId: option.id, optionName: option.name, price: Number(option.price) }]);
    } else {
  ...
  const handleSelectSingle = (group: any, optionId: string) => {
    ...
    setSelected(prev => {
      ...
      return [...filtered, { id: group.id, name: group.name, optionId: option.id, optionName: option.name, price: Number(option.price) }];
    });
  };
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[500px] max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-6 py-4">
            ...
            <div key={group.id} className="space-y-3 border-b pb-4 last:border-0">
              <div className="flex justify-between items-center">
                ...
                {group.isRequired && <span className="text-xs bg-destructive/10 text-destructive px-2 py-1 rounded">Required</span>}
              </div>
                    ...
                    {group.options.map((opt: any) => (
                      <div key={opt.id} className="flex items-center justify-between">
                        <div className="flex items-center space-x-2">
                          ...
                          <Label htmlFor={opt.id} className="font-normal cursor-pointer">{opt.name}</Label>
                        </div>
                        {Number(opt.price) > 0 && <span className="text-sm text-muted-foreground">+{formatCurrency(Number(opt.price))}</span>}
                      </div>
                    ))}
                  </div>
                </RadioGroup>
                  ...
                  {group.options.map((opt: any) => (
                    <div key={opt.id} className="flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        <Checkbox 
                          id={opt.id} 
                          checked={selected.some(s => s.id === group.id && s.optionId === opt.id)}
                          onCheckedChange={(checked) => handleToggleMultiple(group, opt, checked as boolean)}
                        />
                        <Label htmlFor={opt.id} className="font-normal cursor-pointer">{opt.name}</Label>
                      </div>
                      {Number(opt.price) > 0 && <span className="text-sm text-muted-foreground">+{formatCurrency(Number(opt.price))}</span>}
        ...
        <DialogFooter>
          ...
          <Button onClick={handleConfirm} disabled={isLoading}>Add to Cart</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
