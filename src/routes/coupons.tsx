...
import { coupons } from "@/lib/dummy";

export const Route = createFileRoute("/coupons")({
  head: () => ({ meta: [{ title: "Coupons · Grocer.Pro" }] }),
  component: () => (
    <div className="p-4 md:p-6 lg:p-8">
      <DataPage title="Coupons" description="Discount codes redeemable at POS and online." primaryAction={{ label: "New Coupon" }}>
            ...
            <tbody className="divide-y divide-border">
              {coupons.map((c) => (
                <tr key={c.code} className="hover:bg-muted/30">
                  ...
                  <td className="px-4 py-3 text-muted-foreground">{c.uses} / {c.limit}</td>
