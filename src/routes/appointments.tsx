          ...
          <DialogContent className="max-w-md rounded-2xl">
            ...
            <div className="grid gap-4 py-2">
              ...
              <Button
                ...
                disabled={createAppointment.isPending || !formData.customerName || !formData.serviceName}
              >
                {createAppointment.isPending ? <Loader2 className="size-4 mr-2 animate-spin" /> : null}
                Save Appointment
              </Button>
            </div>
          </DialogContent>
      ...
      <div className="space-y-3">
        {isLoading ? (
          ...
          <div className="text-center py-16 border rounded-2xl bg-muted/20 border-dashed border-border/80">
            ...
            <p className="text-muted-foreground mt-1 text-xs">Create your first client appointment to track bookings.</p>
          </div>
        ) : (
              ...
              >
                <div className="flex items-start gap-4">
                  <div className="flex flex-col items-center justify-center p-3 bg-primary/10 text-primary border border-primary/20 rounded-xl min-w-20 sm:min-w-24 shrink-0">
                    ...
                    <span className="text-[10px] text-muted-foreground mt-0.5 font-bold">{duration} min</span>
                  </div>
                    ...
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 text-xs text-muted-foreground mt-1 font-medium">
                      ...
                      {apt.customerPhone && (
                        <span className="flex items-center gap-1">
                          <Phone className="size-3.5 text-muted-foreground/70" /> {apt.customerPhone}
                        </span>
                      )}
                      {apt.staffName && (
                        <span className="flex items-center gap-1 bg-muted/40 px-2 py-0.5 rounded-md border border-border/60">
                          <Clock className="size-3.5 text-muted-foreground/70" /> Stylist: {apt.staffName}
                        </span>
                      )}
                    </div>
                    {apt.notes && (
                ...
                <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-center gap-2 shrink-0 pt-2 sm:pt-0 border-t sm:border-t-0 border-border/60">
                  <Badge
                    className={`text-[10px] font-black uppercase tracking-wider ${
                      isCompleted ? "bg-success/12 text-success border-success/25" :
                      isInProgress ? "bg-info/12 text-info border-info/25" :
                      isCancelled ? "bg-destructive/12 text-destructive border-destructive/25" :
                  ...
                  >
                    ...
                    <SelectContent className="rounded-xl">
                      ...
                      <SelectItem value="in-progress" className="text-xs font-semibold text-info">In Progress</SelectItem>
                      <SelectItem value="completed" className="text-xs font-semibold text-success">Completed</SelectItem>
                      <SelectItem value="cancelled" className="text-xs font-semibold text-destructive">Cancelled</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </div>
