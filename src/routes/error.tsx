...
function ErrorPage() {
  ...
  if (code === 403 || code === 401) {
    title = "Access Denied";
    description = search.message || "You do not have permission to access this resource or perform this action. If you believe this is a mistake, please contact your administrator.";
    Icon = ShieldAlert;
    colorClass = "text-destructive bg-destructive/10";
    shadowClass = "shadow-[0_0_50px_-12px_rgba(255,0,0,0.15)]";
  } else if (code === 404) {
    title = "Not Found";
    description = search.message || "The data or page you were looking for could not be found. It may have been moved, deleted, or you might have an incorrect link.";
    Icon = SearchX;
    colorClass = "text-warning bg-warning/10";
    shadowClass = "shadow-[0_0_50px_-12px_rgba(234,179,8,0.15)]";
  } else if (code >= 500) {
    title = "System Glitch";
    description = search.message || "Our servers encountered an unexpected glitch while processing your request. We have logged the error and are looking into it.";
    Icon = ServerCrash;
    colorClass = "text-destructive bg-destructive/10";
    shadowClass = "shadow-[0_0_50px_-12px_rgba(255,0,0,0.15)]";
  }
  ...
  return (
    <div className="flex min-h-screen w-full items-center justify-center bg-background px-4 relative overflow-hidden">
      ...
      <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] rounded-full opacity-20 blur-3xl pointer-events-none transition-all duration-1000 ${colorClass.split(" ")[1]}`} />
      
      <div className={`relative z-10 w-full max-w-lg rounded-3xl border border-border/50 bg-background/60 p-8 md:p-12 backdrop-blur-xl text-center ${shadowClass}`}>
        
        {/* Animated Icon */}
        <div className="mx-auto mb-6 relative w-24 h-24">
          <div className="absolute inset-0 animate-ping opacity-20 rounded-full bg-current" style={{ color: "inherit" }} />
          <div className={`relative mx-auto flex h-24 w-24 items-center justify-center rounded-3xl border border-current/20 shadow-inner ${colorClass}`}>
