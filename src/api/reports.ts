...
export const getProfitabilityReportFn = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    try {
      ...
      let salesConditions = [eq(schema.sales.organizationId, orgId), eq(schema.sales.status, "completed")];
      if (data.startDate) {
      ...
      const validSales = await db
        .select({ id: schema.sales.id, total: schema.sales.total, discountAmt: schema.sales.discountAmt, taxAmt: schema.sales.taxAmt })
        .from(schema.sales)
      ...
      let totalCogs = 0;
      let productBreakdown: Record<string, { quantitySold: number, revenue: number, cogs: number }> = {};
      ...
      if (saleIds.length > 0) {
        ...
        const saleItems = await db
          ...
          .where(and(inArray(schema.saleItems.saleId, saleIds), eq(schema.saleItems.organizationId, orgId)));
        ...
        const consumptions = await db
          ...
          .innerJoin(schema.inventoryBatches, eq(schema.inventoryBatchConsumptions.batchId, schema.inventoryBatches.id))
          .where(and(inArray(schema.inventoryBatchConsumptions.saleId, saleIds), eq(schema.inventoryBatches.organizationId, orgId)));
      ...
      if (Object.keys(productBreakdown).length > 0) {
        ...
        const nameMap = productsInfo.reduce((acc, p) => ({ ...acc, [p.id]: p.name }), {} as Record<string, string>);
        
        topProducts = Object.entries(productBreakdown).map(([pid, data]) => ({
          productId: pid,
