...
for (const file of files) {
  ...
  const authCheckRegex =
    /const orgId = getCookie\("pos_session_org"\);\s*if \(\!orgId\) return \{ success: false, error: "Unauthorized" \};/g;
  content = content.replace(
    authCheckRegex,
...
`;
  ...
  const createRegex = new RegExp(
    `export const create([A-Za-z]+)Fn = createServerFn\\(\\{ method: "POST" \\}\\)\\s*\\.validator\\(z\\.any\\(\\)( as any)?\\)`,
    "g",
  );
  ...
  const updateRegex = new RegExp(
    `export const update([A-Za-z]+)Fn = createServerFn\\(\\{ method: "POST" \\}\\)\\s*\\.validator\\(z\\.any\\(\\)( as any)?\\)`,
    "g",
  );
  ...
  const deleteRegex = new RegExp(
    `export const delete([A-Za-z]+)Fn = createServerFn\\(\\{ method: "POST" \\}\\)\\s*\\.validator\\(z\\.any\\(\\)( as any)?\\)`,
    "g",
  );
  ...
  const getRegex = new RegExp(
    `export const get([A-Za-z]+)Fn = createServerFn\\(\\{ method: "GET" \\}\\)\\s*\\.validator\\(z\\.any\\(\\)( as any)?\\)`,
    "g",
  );
  ...
  content = content.replace(
    /db\.insert\(schema\.[a-zA-Z]+\)\.values\(data\.([a-zA-Z]+)\)/g,
    (match, dataKey) =>
      match.replace(`data.${dataKey}`, `{...data.${dataKey}, organizationId: session.orgId}`),
  );
