...
for (const file of files) {
  ...
  const validatorRegex = new RegExp(`\\.validator\\(z\\.object\\(\\{ id: z\\.string\\(\\), ${entitySingular}: updateSchema \\}\\)\\)`, 'g');
  content = content.replace(validatorRegex, `.validator(z.object({ id: z.string(), updates: updateSchema.passthrough() }).passthrough())`);
  ...
  const createValidatorRegex = new RegExp(`\\.validator\\(z\\.object\\(\\{ ${entitySingular}: insertSchema \\}\\)\\)`, 'g');
  content = content.replace(createValidatorRegex, `.validator(z.object({ ${entitySingular}: insertSchema.passthrough() }).passthrough())`);

  // Specific manual file fixes:
  if (file === 'customers.ts') {
    content = content.replace('customer: CustomerSchema.partial()', 'updates: CustomerSchema.partial().passthrough()');
    ...
    content = content.replace('z.object({\n  id: z.string(),', 'z.object({\n  id: z.string(),').replace(/z\.string\(\)\.optional\(\)/g, 'z.string().or(z.number()).optional()').replace(/z\.string\(\)\.nullable\(\)\.optional\(\)/g, 'z.string().or(z.number()).nullable().optional()');
  } else if (file === 'products.ts') {
    content = content.replace('product: ProductSchema.partial()', 'updates: ProductSchema.partial().passthrough()');
    ...
    content = content.replace('const updateData: any = { ...data.product,', 'const updateData: any = { ...data.updates,');
  } else if (file === 'users.ts') {
    content = content.replace('user: UserSchema.partial()', 'updates: UserSchema.partial().passthrough()');
    ...
    content = content.replace('invitation: z.object({', 'token: z.string(), role: z.string(), permissions: z.array(z.string()).optional(), invitation: z.object({').replace(/token: z\.string\(\),\n      role: z\.string\(\),/g, ''); // Fix createInvitationFn shape roughly, or just rewrite it fully below
  } else if (file === 'suppliers.ts') {
    content = content.replace('updates: SupplierSchema.partial()', 'updates: SupplierSchema.partial().passthrough()');
    content = content.replace('supplier: SupplierSchema', 'supplier: SupplierSchema.passthrough()');
  } else if (file === 'sales.ts') {
    content = content.replace('sale: SaleSchema.partial()', 'updates: SaleSchema.partial().passthrough()');
    ...
    content = content.replace('items: z.array(PurchaseItemSchema).optional()', 'items: z.array(PurchaseItemSchema).optional(), lines: z.array(z.any()).optional()');
    content = content.replace('purchase: PurchaseSchema', 'purchase: PurchaseSchema.passthrough()');
    content = content.replace('purchaseReturn: PurchaseReturnSchema', 'purchaseReturn: PurchaseReturnSchema.passthrough()');
    content = content.replace('status: z.string()', 'id: z.string(), updates: z.object({ status: z.string() }).passthrough()');
